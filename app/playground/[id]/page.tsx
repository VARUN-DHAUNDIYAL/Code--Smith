...
const MainPlaygroundPage: React.FC = () => {
  ...
  // Keep a ref of templateData so useCallback closures always read the latest
  // value without triggering re-renders (avoids Zustand getState() anti-pattern)
  ...
  const writeFileSync = useCallback(async (_filePath: string, _content: string) => {
    // WebContainer syncing is optional; database persistence remains the source of truth.
  }, []);
  const instance = null;
  ...
  const handleSave = useCallback(
    async (fileId?: string) => {
      ...
      try {
        ...
        // Renamed from updateFileContent → updateContentInTree to avoid shadowing the hook function
        const updatedTemplateData = JSON.parse(
          JSON.stringify(latestTemplateData)
        );
        const updateContentInTree = (items: any[]): any[] =>
          items.map((item) => {
        ...
        updatedTemplateData.items = updateContentInTree(
