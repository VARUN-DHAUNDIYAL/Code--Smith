...
const TerminalComponent = forwardRef<TerminalRef, TerminalProps>(({ 
  ...
  const executeCommand = useCallback(async (command: string) => {
    ...
    try {
      ...
      if (command.trim() === "help") {
        ...
        term.current!.writeln("  cat       - Show file contents");
        term.current!.writeln("  npm run   - Show available npm scripts");
        writePrompt();
        return;
      }
      ...
      if (command.trim() === "ls") {
        try {
          ...
          term.current!.writeln(files.join(' '));
        } catch (error) {
          term.current!.writeln(`Error listing files: ${error}`);
        }
        writePrompt();
        return;
      }
      ...
      if (command.trim().startsWith("cat ")) {
        ...
        try {
          const content = await webContainerInstance.fs.readFile(filename, 'utf8');
          term.current!.writeln(content);
        } catch (error) {
          term.current!.writeln(`Error reading file: ${error}`);
        }
        writePrompt();
        return;
      }

      if (command.trim() === "npm run") {
        try {
          const packageJson = await webContainerInstance.fs.readFile('package.json', 'utf8');
          const pkg = JSON.parse(packageJson);
          const scripts = Object.keys(pkg.scripts || {});
          if (scripts.length > 0) {
            term.current!.writeln("Available scripts:");
            scripts.forEach(script => {
              term.current!.writeln(`  ${script}: ${pkg.scripts[script]}`);
            });
          } else {
            term.current!.writeln("No scripts found in package.json");
          }
        } catch (error) {
          term.current!.writeln(`Error reading package.json: ${error}`);
        }
        writePrompt();
        return;
      }
  ...
  const initializeTerminal = useCallback(() => {
    ...
    // Welcome message
    terminal.writeln("🚀 PromptSmith Terminal");
    terminal.writeln("Type 'help' for available commands");
    terminal.writeln("💡 AI-powered development environment");
    writePrompt();
  ...
  const copyTerminalContent = useCallback(async () => {
    ...
    if (term.current) {
      ...
      if (!content) {
        // If nothing is selected, copy the entire buffer
