...
interface ProjectTableProps {
  ...
  onDuplicateProject?: (id: string) => Promise<{ id: string } | void>
  ...
  const handleDuplicateProject = async (project: Project) => {
    ...
    try {
      ...
      if (result && "id" in result && result.id) {
        window.location.href = `/playground/${result.id}`
      }
    } catch (error) {
          ...
          <TableBody>
            {projects.map((project) => (
              <TableRow key={project.id}>
                ...
                <TableCell>
                  <Badge variant="outline" className="bg-cyan-500/15 text-cyan-500 border-cyan-500">
                    {project.template}
                  </Badge>
                </TableCell>
